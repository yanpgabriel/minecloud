...
✅ Já resolvido (2026-08-11, achado ao usar `make publish` de verdade): push pro Gitea via Cloudflare falhava (`413 Payload Too Large` / `unexpected EOF`) — a camada do JDK sozinha tem >300MB, acima do limite de upload da Cloudflare. Endereço do registry (`REGISTRY_IMAGE`) virou configurável via `.env` (`.env.example` documenta as duas opções: DNS público vs IP local pra contornar o limite), lido automaticamente por `Makefile`, `Justfile` e `docker-compose.yml`. `make publish`/`just publish` agora fazem build multi-arquitetura (`linux/amd64` + `linux/arm64/v8`) via `docker buildx`. Validado: `.env` sobrepõe o padrão corretamente nos dois runners (make e just), com e sem o arquivo presente.

- [ ] **CI automático no Gitea Actions**: pendente até você configurar Actions + runner no `gitea.oyan.dev`. Quando tiver, dá pra criar um `.gitea/workflows/publish.yml` reaproveitando o mesmo `docker buildx build --push` do `make publish`.
