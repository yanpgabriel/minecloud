...
if [[ $PAPERMC_VERSION == "" || $PAPERMC_VERSION == "latest" ]]; then
  ...
  PAPERMC_VERSION=$(curl -s "https://api.papermc.io/v2/projects/paper" | jq '.versions[-1]' -r)
...
if [[ -f ~/${PAPERMC_JAR_NAME} && ${SEMPRE_BAIXAR} == FALSE ]]; then
  ...
  LATEST_DOWNLOAD=$(curl -s "https://api.papermc.io/v2/projects/paper/versions/${PAPERMC_VERSION}/builds/${PAPERMC_BUILD}" | jq '.downloads.application.name' -r)
